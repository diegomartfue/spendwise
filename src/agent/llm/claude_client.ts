...
// Outputs JSON matching MCP tool schemas. Drop-in replacement for gemini_client.ts
...
function get_client(): Anthropic {
  if (!_client) {
    const key = process.env.CLAUDE_API_KEY;
    if (!key) throw new Error("CLAUDE_API_KEY is not set in .env");
...
export async function infer_action(
  ...
  const response = await get_client().messages.create({
  ...
  try {
    ...
    if (parsed.action === "check_balance" && parsed.args) {
      ...
      if (
        typeof a.account === "string" &&
        ACCOUNTS.includes(a.account as account_type)
      ) {
        return {
    ...
    console.error("[Claude] Failed to parse JSON:", message);
    return {
      action: "unknown",
